from flask import jsonify, Blueprint, request #Blueprint ajuda a dar um bom nome para as rotas
...
def create_link(tripId):
    conn = db_connection_handler.get_connection()
    trip_repository = LinksRepository(conn)

    controller = LinkCreator(trip_repository)
...
def find_trip_link(tripId):
    ...
    controller = LinkFinder(trip_repository)
